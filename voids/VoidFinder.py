...
find_void_catalogs = True # If true, finds void catalogs
calc_void_properties = False # if true, caclaulttes void effective radii, etc.

run_sdss = False #create sdss catalogs
run_desi = True # create desi catalogs
...
if run_desi:
    ...
    survey_names = ['DESIVAST_RedBlue_NGC_','DESIVAST_RedBlue_SGC_']
    ...
    # Input file name
    # File format: RA, dec, redshift, comoving distance, absolute magnitude
    galaxies_filenames = ['loa_redblue_ngc.fits', 'loa_redblue_sgc.fits']
    ...
    for magnitude_limit in magnitude_limits:
        ...
        out_directory = f'./loa/minus{np.abs(magnitude_limit)}/'
...
def prepare_catalog (voids_path, galaxies):
    if os.path.exists(voids_path):
        ...
        vfc.calculate_r_eff(overwrite=True)
        print('vflag_eff:',voids_path)
        vfc.calculate_vflag(vflag_path = voids_path[:-11]+'vflag.fits',
                            overwrite=True) #Restarting the VoidFinding won't clear the previous vflag output
